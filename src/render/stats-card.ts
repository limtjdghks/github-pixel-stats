...
function renderMetric(label: string, value: number, icon: MetricIcon, x: number, width: number, accent: string): string {
  ...
  const numberSize = formatted.length > 5 ? 36 : formatted.length > 3 ? 48 : 64;
  return `<g>
${renderMetricIcon(icon, x + 12, 141, accent)}
<text x="${textX}" y="181" font-size="${numberSize}" font-weight="700">${formatted}</text>
<text x="${textX + Math.floor((width - 65) / 2)}" y="205" text-anchor="middle" font-size="20" class="muted">${label}</text>
</g>`;
}
