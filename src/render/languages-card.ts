...
function renderLanguage(language: LanguageStat, index: number): string {
  ...
  const segments = Array.from({ length: LANGUAGE_BAR_SEGMENTS }, (_, segment) =>
    `<rect data-language-segment="true" x="${196 + segment * 14}" y="${y + 17}" width="12" height="15" fill="${segment < language.filledSegments ? color : CARD_COLORS.track}"/>`,
  ).join("");
  const fontSize = language.name.length > 12 ? 14 : 20;
  return `<g>
...
export function renderLanguagesCard(snapshot: StatsSnapshot): string {
  ...
  const description = snapshot.languages.length
    ? snapshot.languages.map((language, index) => `${index + 1}. ${language.name}: ${formatCount(language.commitCount)} commits (${(language.share * 100).toFixed(2)}% of all language commits)`).join("; ")
...
<text x="51" y="33" font-size="20" textLength="243" lengthAdjust="spacingAndGlyphs" font-weight="700" class="light">MOST COMMITTED LANGUAGES</text>
